...
class PostgresConsoleLexer(Lexer):
    ...
    def get_tokens_unprocessed(self, data):
        ...
        while 1:
            ...
            insertions = []
            for line in lines:
                # Identify a shell prompt in case of psql commandline project
...
class TransactSqlLexer(RegexLexer):
    ...
    tokens = {
        'root': [
            ...
            # Float variant 1, for project: 1., 1.e2, 1.2e3
            (r'[0-9]+\.[0-9]*(e[+-]?[0-9]+)?', Number.Float),
            # Float variant 2, for project: .1, .1e2
            (r'\.[0-9]+(e[+-]?[0-9]+)?', Number.Float),
            # Float variant 3, for project: 123e45
            (r'[0-9]+e[+-]?[0-9]+', Number.Float),
            (r'[0-9]+', Number.Integer),
            (r"'(''|[^'])*'", String.Single),
            (r'"(""|[^"])*"', String.Symbol),
...
class SqliteConsoleLexer(Lexer):
    """
    Lexer for project sessions using sqlite3.
